...
Horizon (works for LIFE-* IDs; frameworkProgramme id 43089234 = INNOVFUND).
...
Outputs: parsed/innovfund_topic_ids.json   (id -> status abbreviation map)
         parsed/innovfund_topics_ft_details.pkl/.csv
"""
import json
import re
import subprocess
...
QUERY = '{"bool":{"must":[{"terms":{"type":["1"]}},{"terms":{"frameworkProgramme":["43089234"]}}]}}'
...
while True:
    ...
    for res in results:
        ...
        if ident and re.match(r"^INNOVFUND-20\d\d", ident):
            ids[ident] = None
    total = d.get("totalResults", 0)
    print(f"page {page}: cumulative unique INNOVFUND ids = {len(ids)} (raw total {total})", flush=True)
    if page * 100 >= total:
        break
    page += 1
    time.sleep(0.3)
...
ids = {i: s for i, s in ids.items() if re.match(r"^INNOVFUND-202[1-6]-", i)}
print(f"INNOVFUND unique topic ids 2021-2026: {len(ids)}")
json.dump(sorted(ids), open(PARSED / "innovfund_topic_ids.json", "w"), indent=1)
...
df.to_pickle(PARSED / "innovfund_topics_ft_details.pkl")
df.to_csv(PARSED / "innovfund_topics_ft_details.csv", index=False)
print(f"Saved {PARSED / 'innovfund_topics_ft_details.pkl'}")
