"""Fetch F&T Portal topicDetails JSON for all CL4 (scoped) topic IDs.

ID sources (union):
  - parsed/cl4_topics_from_cordis.pkl      (CORDIS-known, signed topics)
  - parsed/cl4_all_topic_ids.json          (full list from grantsTenders.json,
                                            incl. 2025/2026 not-yet-signed; optional)

Reuses fetch_topic / parse_topic / cache dir from fetch_topic_details.py.
Output: parsed/cl4_topics_ft_details.pkl/.csv
"""
import json
import sys
import time
...
ids = set(pd.read_pickle(PARSED / "cl4_topics_from_cordis.pkl")["topic_id"])
extra = PARSED / "cl4_all_topic_ids.json"
if extra.exists():
...
df.to_pickle(PARSED / "cl4_topics_ft_details.pkl")
df.to_csv(PARSED / "cl4_topics_ft_details.csv", index=False)
print(f"Saved {PARSED / 'cl4_topics_ft_details.pkl'}")
